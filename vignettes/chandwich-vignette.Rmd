...
The function `conf_intervals` calculates (profile, if necessary) likelihood-based confidence intervals for individual parameters.  Of course, in this one-dimensional example no profiling of the loglikelihood is performed.
...
We could examine the significance of the quadratic term using an adjusted likelihood ratio test.  If \code{approx = FALSE} (the default) then equation (17) in Section 3.3 of @CB2007 is used.  If \code{approx = TRUE} the approximation in equation (18) is used.

```{r}
compare_models(pois_quadratic, pois_linear)$p_value
compare_models(pois_quadratic, pois_linear, approx = TRUE)$p_value
```

The $p$-value based on equation (17) agrees with the value in the *sandwich* vignette, which is based on a Wald test, and the $p$-value based on equation (18) is only slightly different.
...
full <- adjust_loglik(gev_loglik, data = owtemps, init = init,
                      par_names = par_names)
# Rows 1, 3 and 4 of Table 2 of Chandler and Bate (2007)
round(attr(full, "MLE"), 4)
round(attr(full, "SE"), 4)
round(attr(full, "adjSE"), 4)
```
...
```{r}
# Confidence intervals
conf_intervals(full)$prof_CI
```

### Confidence regions

We produce a plot akin Figure 4(a) of @CB2007, but including an extra 95\% confidence region for the vertical adjustment.

```{r, fig.align='center', fig.width=7, fig.height=7}
which_pars <- c("mu[0]", "mu[1]")
gev_none <- conf_region(full, which_pars = which_pars, type = "none")
gev_vertical <- conf_region(full, which_pars = which_pars)
gev_cholesky <- conf_region(full, which_pars = which_pars, type = "cholesky")
gev_spectral <- conf_region(full, which_pars = which_pars, type = "spectral")
plot(gev_none, gev_cholesky, gev_vertical, gev_spectral, lwd = 2, 
     xlim = c(80, 82.25), ylim = c(1.75, 3.5))
```

The 95\% contours of the profile adjusted loglikelihoods are virtually identical.

### Comparing nested models

Suppose that we wish to test the null hypothesis that Oxford and Worthing share a common GEV shape parameter, that is, that $\xi_1 = 0$.  In the misspecified Poisson model example we called `adjust_loglik` to perform loglikelihood adjustment under a reduced model and then used `compare_models` to carry out an adjusted likelihood ratio test.  Alternatively, we can use a single call to `compare_models`.

```{r}
compare_models(full, fixed_pars = "xi[1]")$p_value
compare_models(full, fixed_pars = "xi[1]", approx = TRUE)$p_value
```
